...
const ConsultationBooking: React.FC<ConsultationBookingProps> = ({ isOpen, onClose, mode = 'popup' }) => {
      ...
      <div className={wrapperClasses}>
        ...
        >
          ...
          <button
            ...
            className="w-full px-6 py-3 bg-blue-900 hover:bg-blue-800 text-white font-semibold rounded-xl transition-all shadow-lg shadow-blue-900/20"
          >
            Done
          </button>
  ...
  return (
    <div className={wrapperClasses}>
      <motion.div
        initial={mode === 'popup' ? { opacity: 0, scale: 0.95 } : { opacity: 0, y: 10 }}
      ...
      >
        ...
        {mode === 'popup' && (
          <button
            onClick={() => { resetForm(); onClose(); }}
            className="absolute top-3 right-3 z-50 p-2 bg-white hover:bg-red-50 text-gray-400 hover:text-red-500 rounded-full transition-colors shadow-sm border border-gray-100"
          >
            <X className="w-4 h-4" />
          </button>
        )}
        ...
        {consultationType && (
          <motion.div
            ...
            className={`
              hidden md:flex md:w-64 bg-slate-900 text-white p-5 flex-col flex-shrink-0
            ...
            <div className="space-y-3 mt-auto">
              <div className="border-t border-slate-800 pt-3">
                ...
                <div className="flex items-center space-x-3">
                   ...
                   <span className="text-xs text-slate-300">Cereforge Meet</span>
                </div>
              </div>
                ...
                >
                  {consultationTypes.map((type) => (
                    <motion.div
                      ...
                      className="p-4 rounded-xl border border-gray-100 bg-white hover:shadow-md cursor-pointer transition-all group flex items-center gap-4"
                    >
                      <div className={`w-10 h-10 bg-gradient-to-br ${type.color} rounded-lg flex items-center justify-center text-lg text-white shadow-sm flex-shrink-0`}>
                        {type.icon}
                      </div>
                ...
                >
                  ...
                  <div className="flex-1 max-w-[340px] flex flex-col mx-auto lg:mx-0">
                    <button
                      ...
                      className="w-full mb-4 px-3 py-2 bg-gray-50 hover:bg-gray-100 rounded-lg text-left transition-colors flex items-center justify-between text-xs font-medium text-gray-600 border border-transparent hover:border-gray-200"
                    >
                    ...
                    <AnimatePresence>
                      {showTimezoneDropdown && (
                        <motion.div 
                          ...
                          className="mb-4 overflow-hidden border border-gray-100 rounded-lg shadow-sm"
                        >
                          <div className="p-2 bg-gray-50">
                             <input
                               ...
                               placeholder="Search..."
                               className="w-full px-2 py-1.5 text-xs bg-white border border-gray-200 rounded focus:outline-none focus:border-blue-500"
                             />
                          </div>
                          <div className="max-h-32 overflow-y-auto">
                            {filteredTimezones.map(tz => (
                              <button
                                ...
                                className="w-full text-left px-3 py-2 text-xs hover:bg-blue-50 text-gray-700 truncate"
                              >
                                {tz.label}
                              </button>
                            ))}
                    ...
                    <div className="bg-white border border-gray-100 rounded-xl p-4 shadow-sm">
                      <div className="flex items-center justify-between mb-4 px-1">
                        <button onClick={handlePrevCalendarMonth} className="p-1 hover:bg-gray-50 rounded-full text-gray-400 hover:text-gray-600 transition-colors">
                          <ChevronLeft className="w-4 h-4" />
                        </button>
                        <span className="text-sm font-bold text-gray-800">
                          {dayjs(new Date(dayjs().year(), currentCalendarMonth)).format('MMMM YYYY')}
                        </span>
                        <button onClick={handleNextCalendarMonth} className="p-1 hover:bg-gray-50 rounded-full text-gray-400 hover:text-gray-600 transition-colors">
                          ...
                          return (
                            <button
                              ...
                              className={`
                                ...
                                ${isSelected ? 'bg-blue-600 text-white shadow-md scale-100 z-10' : ''}
                                ${!isSelected && isAvailable ? 'text-gray-700 hover:bg-blue-50 hover:text-blue-600' : ''}
                  ...
                  {selectedDate && (
                    <motion.div
                      ...
                      animate={{ opacity: 1, x: 0 }}
                      className="w-full lg:w-40 flex flex-col border-t lg:border-t-0 lg:border-l border-gray-100 pt-4 lg:pt-0 lg:pl-6"
                    >
                      <h4 className="text-xs font-bold text-gray-900 mb-3 sticky top-0 bg-white z-10 py-1">
                      ...
                      <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar space-y-2 max-h-[350px]">
                        {timeSlots.map((time) => (
                          <button
                            ...
                            className={`
                              w-full px-3 py-2 rounded-lg text-xs font-semibold border transition-all
                              ${selectedTime === time 
              ...
              <div className="w-24 flex justify-end">
                {step === 3 && (
                  <motion.button
                    ...
                    className="flex items-center space-x-2 px-4 py-2 bg-blue-900 hover:bg-blue-800 text-white text-xs font-bold rounded-lg shadow-sm transition-all disabled:opacity-50 disabled:shadow-none"
                  >
                    <span>Confirm</span>
